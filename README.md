...
- Opens `*.linxtrace` with required sidecar `*(in-band META)`.
...
- Includes high-performance interaction controls (zoom-anchor, drag-pan, keyboard navigation, hover details).
...
```bash
cd /Users/zhoubot/LinxCoreSight
node scripts/linxtrace_cli.js lint /path/to/trace.linxtrace
node scripts/linxtrace_cli.js stats /path/to/trace.linxtrace
node scripts/linxtrace_cli.js schema-check /path/to/trace.linxtrace
node scripts/linxtrace_cli.js first-failure /path/to/trace.linxtrace
node scripts/linxtrace_cli.js render-check /path/to/trace.linxtrace
node scripts/trace_lint.js /path/to/trace.linxtrace
node scripts/trace_perf_check.js /path/to/trace.linxtrace --max-ms 5000
```
...
Pipeline trace contract skill:
- `/Users/zhoubot/.codex/skills/linx-trace-pipeview/SKILL.md`
